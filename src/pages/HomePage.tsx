...
export function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-[#2D5BE3]">

      {/* NAV */}
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <span className="font-black text-[#2D5BE3] text-lg sm:text-xl tracking-tight">
            📋 Yıllık Plan
          </span>
          <a
            href="#form"
            className="text-sm font-semibold text-[#2D5BE3] bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-lg transition"
          >
            Erken Erişim
          </a>
        </div>
      ...
      <section className="bg-white pt-16 pb-20 px-4">
        <div className="max-w-3xl mx-auto text-center flex flex-col items-center">
          <span className="inline-block bg-[#2D5BE3]/10 text-[#2D5BE3] text-xs font-bold px-4 py-1.5 rounded-full mb-6 tracking-widest uppercase">
            Öğretmenler için ücretsiz
          </span>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-[#2D5BE3] leading-tight mb-6">
          ...
          <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-4 justify-center">
            <button
              ...
              className="w-full sm:w-auto bg-[#F59E0B] hover:bg-[#F59E0B]/90 text-white font-bold text-lg px-8 py-4 rounded-xl transition shadow-lg shadow-[#F59E0B]/30"
            >
              Hemen Başla
            </button>
            <button
              onClick={() => navigate('/yukle')}
              className="w-full sm:w-auto border-2 border-[#2D5BE3] text-[#2D5BE3] hover:bg-[#2D5BE3] hover:text-white font-bold text-lg px-8 py-4 rounded-xl transition"
            >
              Dosyadan Yükle
            </button>
          </div>
      ...
      <section id="features" className="py-20 px-4 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-black text-[#2D5BE3] mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map(({ emoji, title, desc }) => (
              <div
                key={title}
                className="bg-[#FAFAF9] rounded-2xl p-8 border border-[#E7E5E4] shadow-[0_1px_3px_rgba(0,0,0,0.06)] hover:shadow-md hover:border-[#F59E0B]/30 transition"
              >
                <div className="text-4xl mb-6">{emoji}</div>
                <h3 className="text-[#2D5BE3] font-black text-lg mb-3">{title}</h3>
      ...
      <section id="form" className="py-20 px-4 bg-white">
        <div className="max-w-3xl mx-auto bg-[#2D5BE3] rounded-3xl p-6 sm:p-12 shadow-[0_4px_16px_rgba(0,0,0,0.12)] relative overflow-hidden">
      ...
      <footer className="border-t border-slate-200 py-10 px-4 text-center items-center flex flex-col gap-2 bg-slate-50">
        <div className="font-black text-[#2D5BE3]">📋 Yıllık Plan</div>
        <p className="text-slate-500 text-sm">© {new Date().getFullYear()} Tüm hakları saklıdır.</p>
        <p className="text-slate-400 text-xs text-center">Türk öğretmenler için, <span className="text-[#F59E0B] font-bold">ücretsiz</span>.</p>
      </footer>
    </div>
  )
}
