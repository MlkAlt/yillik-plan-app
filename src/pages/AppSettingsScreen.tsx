...
export function AppSettingsScreen({ onPlanEkle, onPlanSil, user, planlar: planlarProp = [] }: AppSettingsScreenProps) {
  ...
  function handleKaydet() {
    const a = readAyarlar();
    localStorage.setItem(StorageKeys.OGRETMEN_AYARLARI, JSON.stringify({ ...a, adSoyad, okulAdi, yil }));
    setDegisti(false);
  ...
  function handleYeniPlanEkle(entries: PlanEntry[]) {
    ...
    setMufredatUyari(eksik.length > 0 ? eksik.join(', ') + ' için ders içeriği bulunamadı.' : '');
    onPlanEkle(entries);
    ...
    <div className="max-w-lg mx-auto p-4 w-full pb-20">
      ...
      <Card className="mb-3">
        ...
        {planlarProp.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">Henüz plan yok</p>
        ) : (
      ...
      <Card className="mb-3">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Tercihler</p>        <div className="flex flex-col gap-4">
          ...
          {isBildirimDestekleniyor() && (
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#1C1917]">Haftalık hatırlatıcı</p>
                <p className="text-xs text-gray-400 mt-0.5">
                  {bildirimIzni === 'denied' ? 'Tarayıcı ayarlarından izin ver' : 'Kazanımı haftabaşı bildir'}
                </p>
              </div>
