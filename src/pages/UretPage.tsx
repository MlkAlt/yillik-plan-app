...
const ARACLAR = [
  { id: 'sinav',    ikon: FileText,  ad: 'Yazili Sinav',    alt: 'Kazanim bazli, cevap anahtarli',  renk: '#4F6AF5' },
  { id: 'etkinlik', ikon: Target,    ad: 'Sinif Etkinligi', alt: 'Grup, bireysel, oyun bazli',       renk: '#059669' },
  { id: 'materyal', ikon: BookOpen,  ad: 'Ders Materyali',  alt: 'Calisma yapragi, kavram haritasi', renk: '#6D28D9' },
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
        <div style={{ padding: '16px 20px 12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display),'Bricolage Grotesque',sans-serif", fontSize: '20px', fontWeight: 800, color: 'var(--color-text1)', letterSpacing: '-0.03em' }}>{selectedTool.ad} Uret</h1>
        </div>
        ...
        <div style={{ padding: '0 20px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text2)', marginBottom: '6px' }}>Sinif</p>
          ...
          <div>
            <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text2)', marginBottom: '6px' }}>Soru Sayisi</p>
          ...
          <div>
            <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text2)', marginBottom: '6px' }}>Soru Turu</p>
            <select value={soruTuru} onChange={e => setSoruTuru(e.target.value)} style={{ width: '100%', height: '44px', padding: '0 14px', borderRadius: '12px', border: '1.5px solid var(--color-border)', background: 'var(--color-surface)', fontSize: '15px', color: 'var(--color-text1)', outline: 'none', appearance: 'none', fontFamily: 'inherit' }}>
              <option>Karma</option>
              <option>Sadece Coktan Secmeli</option>
              <option>Sadece Acik Uclu</option>
            </select>
          </div>
          <div>
          ...
          <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: '12px', padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#92400E', fontWeight: 500 }}>Bu uretim 1 jeton kullanacak. Bakiyeniz: {BAKIYE} jeton.</p>
          </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      <div style={{ padding: '16px 20px 8px' }}>
        <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-text3)', marginBottom: '4px' }}>Uret</p>
        <h1 style={{ fontFamily: "var(--font-display),'Bricolage Grotesque',sans-serif", fontSize: '24px', fontWeight: 800, color: 'var(--color-text1)', letterSpacing: '-0.04em' }}>AI destekli - Jeton ile</h1>
      </div>
      <div style={{ padding: '8px 16px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ borderRadius: '20px', background: 'linear-gradient(145deg,#1B2E5E,#243A78)', padding: '18px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
            <div>
              <p style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,.55)', marginBottom: '4px' }}>Jeton Bakiyesi</p>
            ...
            <button style={{ height: '36px', padding: '0 14px', borderRadius: '100px', background: 'rgba(255,255,255,.12)', border: '1px solid rgba(255,255,255,.16)', color: '#fff', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>+ Jeton Al</button>
          </div>
          ...
          <p style={{ fontSize: '11px', color: 'rgba(255,255,255,.45)' }}>Bu ay 3 ucretsiz jeton hediye edildi</p>
        </div>
        <div>
            ...
            {ARACLAR.map(arac => {
              ...
              return (
                <button key={arac.id} type="button" onClick={() => setSelectedId(arac.id)} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '16px', textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <span style={{ fontSize: '11px', fontWeight: 700, color: '#D97706' }}>1 jeton</span>
                </button>
              )
            })}
          </div>
        ...
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '16px', textAlign: 'center' }}>
          <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-text1)', marginBottom: '4px' }}>Sinirsis mi istiyorsunuz?</p>
          <p style={{ fontSize: '12px', color: 'var(--color-text2)', marginBottom: '12px' }}>149 TL/ay ile tum araclar sinirsis.</p>
          <button style={{ width: '100%', height: '44px', borderRadius: '100px', background: '#4F6AF5', color: '#fff', border: 'none', fontSize: '15px', fontWeight: 600, cursor: 'pointer' }}>Premium a Gec</button>
        </div>
      </div>
    </div>
  )
